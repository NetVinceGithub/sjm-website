...
const Contributions = () => {
  ...
  const extractAvailableFilters = () => {
    ...
    setAvailableMonths(Array.from(monthSet).sort((a, b) => a - b));
  ...
  const getFilteredContributions = () => {
    ...
    if (selectedMonth || selectedYear) {
      filteredEmployees = filteredEmployees.filter((employee) => {
        ...
        const date = new Date(employee.date);
        const empMonth = date.getMonth() + 1;
        const empYear = date.getFullYear();

        const monthMatch = !selectedMonth || empMonth === parseInt(selectedMonth);
        const yearMatch = !selectedYear || empYear === parseInt(selectedYear);
...
const employeeData = filteredData.employees.map((employee) => ({
  ...
  employerSSSshare: employee.contributions.sss.total / 2,
  employerPagibigShare: employee.contributions.pagibig.total / 2,
}));
...
const columns = [
  ...
  },
  {
    ...
    selector: (row) => `₱${(row.sss / 2 || 0).toLocaleString()}`,
    sortable: true,
    width: "150px",
  },
  {
    ...
    selector: (row) => `₱${(row.philhealth / 2 || 0).toLocaleString()}`,
    sortable: true,
    width: "150px",
  },
  {
  ...
  },
  {
    ...
    selector: (row) => `₱${(row.pagibig / 2 || 0).toLocaleString()}`,
    sortable: true,
    width: "150px",
  },
];
