...
const Logins = () => {
  ...
  const fetchUserActivities = async (userId, sessionId) => {
    try {
      ...
      const response = await axios.get(
        `${import.meta.env.VITE_API_URL}/api/activity/user-activities/${userId}?sessionId=${sessionId}`
      );
  ...
  const ActivityRow = ({ activities }) => {
    if (!activities || activities.length === 0) {
      return (
        <div className="p-4 bg-gray-50 border-t">
          <p className="text-sm text-gray-500 italic">No activities recorded for this session</p>
        </div>
      );
    }
    ...
    return (
      <div className="p-4 bg-gray-50 border-t">
        <h4 className="font-semibold text-sm text-gray-700 mb-3">Session Activities:</h4>
        <div className="space-y-2">
          {activities.map((activity, index) => (
            <div key={index} className="flex items-center justify-between bg-white p-3 rounded border text-sm">
              <div className="flex-1">
                <span className="font-medium text-gray-800">{activity.action}</span>
                {activity.resource && (
                  <span className="text-gray-600 ml-2">on {activity.resource}</span>
                )}
                {activity.details && (
  ...
  const columns = [
    ...
    },
    {
      ...
      cell: row => (
        <span
          className={`px-3 py-1 rounded-full text-xs ${row.role === 'Approver'
