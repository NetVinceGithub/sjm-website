...
const FilterComponent = ({ show, onClose, onSchedulesSelected }) => {
  ...
  const handleRemoveSchedule = (keyToRemove) => {
    ...
    setScheduleOptions((prev) =>
      prev.filter((option) => option.key !== keyToRemove)
    );
  ...
  const handleApplyFilters = () => {
    ...
    const formattedSchedules = selectedSchedules.map((schedule) => ({
      ...
      color:
        scheduleOptions.find((opt) => opt.key === schedule.key)?.color ||
        "bg-gray-500",
    }));
  ...
  return (
    ...
    <div className="fixed top-28 right-8 z-50">
      <div className="w-80 bg-white border rounded-lg shadow-sm max-h-[90vh]">
        {/* Modal content */}
        <div className="w-80 bg-white border border-gray-200 rounded-lg shadow-lg max-h-[90vh]">
          {/* Header */}
          <div className="flex items-center justify-between p-2 border-b border-gray-200">
            <h3 className="font-medium flex mt-2 text-sm text-gray-900">
              Filters
            </h3>
            <X
            ...
            {isOpen.schedules && (
              <div className="px-4 pb-4 space-y-3">
                    ...
                    >
                      <label className="flex text-sm items-center space-x-2 cursor-pointer flex-1">
                        ...
                        <div className="flex items-center space-x-2">
                          <div
                            className={`w-3 h-3 rounded ${option.color}`}
                          ></div>
                ...
                <button
                  ...
                  className="w-full h-8 flex items-center justify-center space-x-2 p-2 border border-dashed border-gray-300 rounded-md hover:border-gray-400 hover:bg-gray-50 text-gray-600"
                >
          ...
          <div className="p-4 space-y-2">
            <button
              ...
              className="w-full h-8 px-4 py-2 ustify-center items-center text-sm font-medium text-neutralDGray border rounded-md hover:bg-red-400 hover:text-white transition-all duration-300 focus:outline-none focus:ring-2 focus:ring-blue-500"
            >
              Clear filters
            </button>
            <button
              onClick={handleApplyFilters}
              className="w-full h-8 flex justify-center items-center px-4 py-2 text-sm font-medium text-neutralDGray border rounded-md hover:bg-green-400 hover:text-white transition-all duration-300 focus:outline-none focus:ring-2 focus:ring-blue-500"
            >
              Apply Filters
            </button>
          </div>
