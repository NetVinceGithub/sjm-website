...
export default function BulkEmployeeMessageModal({
  show,
  isOpen,
  handleCloseBulk,
  onClose,
  ...
  const fetchEmployees = async () => {
    ...
    try {
      ...
      // Filter out employees with RESIGNED status
      const activeEmployees = employeeList.filter((employee) => {
      ...
      console.log(
        `Filtered ${
          employeeList.length - activeEmployees.length
        } resigned employees from the list`
      );
    } catch (error) {
  ...
  const handleFileChange = (event) => {
    ...
    if (currentTotalSize + newTotalSize > maxTotalSize) {
      alert(
        "Total file size would exceed 500MB limit. Please remove some files or choose smaller files."
      );
      return;
    }
  ...
  const handleSendMessage = async () => {
    ...
    console.log(
      "Selected Employees Full Data:",
      employees.filter((emp) => selectedEmployees.includes(emp.id))
    );
    console.log(
      "Attachments:",
      attachments.map((f) => ({ name: f.name, size: f.size }))
    );
    ...
    try {
      ...
      if (response.data.success) {
        alert(
          `Message sent successfully to ${response.data.sentCount} employees!`
        );
        ...
        <div className="flex flex-col">
          <div className="w-full max-w-3xl bg-white p-6 border border-gray-300 rounded-md shadow-md min-h-[500px]">
            ...
            <div className="mb-6 relative">
              <label
                htmlFor="subject"
                className="block text-xs font-medium text-gray-700 -mt-4 mb-2"
              >
              ...
              <label
                htmlFor="message"
                className="block text-xs font-medium text-gray-700 mb-2 mt-3"
              >
              ...
              <div className="mb-4 -mt-10 ml-2">
                ...
                <div className="space-y-1 max-h-24 overflow-y-auto border border-gray-200 rounded p-2 bg-gray-50">
                  {attachments.map((file, index) => (
                    <div
                      key={index}
                      className="flex items-center justify-between gap-2 text-[11px] text-gray-600 bg-white p-1 rounded"
                    >
            ...
            <p className="text-xs -mt-5 text-red-400 text-center italic mb-3">
              **Note: You can batch message or you can message employee
              individually.**
            </p>
              ...
              <div className="flex justify-between mb-3">
                <div>
                  <h5 className="text-gray-600 text-sm italic">
                    List of Active Employees ({selectedEmployees.length}{" "}
                    selected)
                  </h5>
                </div>
