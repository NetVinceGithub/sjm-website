                                  ...
                                  {/* {filteredInvoices
                                    .filter(
                                      (invoice, index, self) =>
                                        index ===
                                        self.findIndex(
                                          (inv) => inv.controlNumber === invoice.controlNumber
                                        )
                                    )
                                    .map((invoice) => (
                          ...
                          <div className="">
                            ...
                            <div>
                              <table className="min-w-full -ml-0 table-auto border-separate border border-black text-sm mt-5">
                                ...
                                at the same time const rows = filteredInvoices.map( (invoice, index)
                                => { const dailyRate = parseFloat( invoice.dailyrate || 0 ); const
                                overtimeHours = parseFloat( invoice.totalOvertime || 0 ); const
                                overtimeAmount = parseFloat( invoice.overtimePay || 0 ); const
                                grossPay = parseFloat( invoice.gross_pay || 0 ); const adminFee =
                                grossPay * 0.1; const subtotal = grossPay + adminFee; const vat =
                                subtotal * 0.12; const totalDue = subtotal + vat; // Accumulate
                                totals totalDailyRate += dailyRate; totalOvertimeHours +=
